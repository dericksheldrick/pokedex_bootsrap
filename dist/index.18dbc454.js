...
const pokemonModal = new _bootstrap.Modal(document.getElementById("pokemonModal"));
let offset = 0;
...
const createPokemonCard = (pokemon)=>{
    ...
    cardText.textContent = `Type: ${pokemon.types.map((type)=>type.type.name).join(", ")}`;
...
const showPokemonDetails = async (pokemonId)=>{
    ...
    if (pokemonDetails) {
        document.getElementById("modalImg").src = pokemonDetails.sprites.other["official-artwork"].front_default;
        document.getElementById("modalName").textContent = pokemonDetails.name;
        document.getElementById("modalType").textContent = `Type: ${pokemonDetails.types.map((type)=>type.type.name).join(", ")}`;
        document.getElementById("modalAbilities").textContent = `Abilities: ${pokemonDetails.abilities.map((ability)=>ability.ability.name).join(", ")}`;
        document.getElementById("modalStats").textContent = `Stats: ${pokemonDetails.stats.map((stat)=>`${stat.stat.name}: ${stat.base_stat}`).join(", ")}`;
        pokemonModal.show();
    }
};
...
searchForm.addEventListener("submit", (event)=>{
    ...
    if (pokemonNameOrId) searchPokemon(pokemonNameOrId);
});
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["j2YDk","1SICI"], "1SICI", "parcelRequirebbb8")
